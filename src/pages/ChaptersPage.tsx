 ...
 const getCompletedTasks = (): string[] => {
  ...
  (async () => {
    try {
      ...
      // Use the slug from URL to find course
      const found = getCourseBySlug(courseSlug || '');
      // OR if you don't want to import: courses.find(c => c.slug === courseSlug);
...
}, [courseSlug]); // ← Important: depend on courseSlug
  ...
  return (
      ...
      <div className="min-h-screen bg-background">
        <div className="container mx-auto px-4 py-8 sm:py-12">
          ...
          {/* Course Header */}
          <div className="mb-8 sm:mb-12" data-tour="course-header">
          ...
          <div className="space-y-4 sm:space-y-6">
            <h2
              className="text-xl sm:text-2xl font-heading font-bold text-foreground mb-4 sm:mb-6"
              data-tour="chapters-title"
            >
              Course Chapters
            </h2>
        ...
        <style>{`
          /* ... all your existing mobile/desktop styles ... */
          @media screen and (max-width: 768px) {
            .course-stats { display: flex !important; flex-direction: row !important; justify-content: space-between !important; align-items: center !important; gap: 12px !important; padding: 8px 0 !important; flex-wrap: nowrap !important; }
            .course-stats > div { flex: 1 !important; text-align: center !important; padding: 4px 2px !important; min-width: 0 !important; }
            .course-stats > div > div:first-child { font-size: 16px !important; font-weight: 700 !important; color: #1f2937 !important; }
            .course-stats > div > div:last-child { font-size: 10px !important; color: #6b7280 !important; white-space: nowrap !important; overflow: hidden !important; text-overflow: ellipsis !important; text-transform: uppercase !important; }
            .course-stats > div:last-child > div:first-child { font-size: 18px !important; color: #059669 !important; }
            .course-stats > div:last-child > div:last-child { color: #059669 !important; }
          }
          @media screen and (min-width: 769px) {
            .chapter-card-title { font-size: 1.125rem !important; }
            .chapter-card-description { font-size: 0.95rem !important; }
            .chapter-card-meta { font-size: 0.9rem !important; }
            .lesson-item span { font-size: 0.9rem !important; }
          }
          @media screen and (max-width: 768px) {
            .chapter-card-header { padding: 16px !important; }
            .chapter-card-content { padding: 0 16px 16px 16px !important; }
            .chapter-card-title { font-size: 16px !important; line-height: 1.3 !important; margin-bottom: 8px !important; }
            .chapter-card-description { font-size: 12px !important; margin-bottom: 12px !important; }
            .chapter-card-progress-container { margin-bottom: 12px !important; max-width: 100% !important; }
            .chapter-card-meta { font-size: 12px !important; }
            .chapter-card-button { padding: 12px 16px !important; font-size: 14px !important; }
            .lesson-item { padding: 8px !important; margin-bottom: 4px !important; }
            .lesson-item span { font-size: 13px !important; }
          }
        `}</style>
      </div>
    );
...
/** Sub-components with data-tour attributes **/

function Breadcrumb({ courseName }: { courseName: string }) {
  return (
    <nav className="mb-4 sm:mb-8" data-tour="breadcrumb">
...
function CourseHeader({
  ...
  return (
    <>
      ...
      <div className="course-stats flex flex-wrap sm:grid sm:grid-cols-4 gap-2 sm:gap-6" data-tour="course-stats">
...
function ChapterCard({
 ...
 course,
  ...
  const estimatedTime = chapter.lessons.length * 15;
  ...
  return (
    ...
    >
      <CardHeader className="chapter-card-header p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row items-start justify-between gap-3 sm:gap-4">
          <div className="flex items-start gap-3 sm:gap-4 flex-1 w-full min-w-0">
            <div className="flex-1 min-w-0">
              ...
              <div className="chapter-card-progress-container max-w-full sm:max-w-md mb-3 sm:mb-4">
                <ProgressBar value={chapterProgress.completionPercentage} label="Chapter Progress" size="sm" />
              </div>
          ...
          <div className="flex flex-col items-center sm:items-end gap-2 w-full sm:w-auto mt-4 sm:mt-0">
            <Link to={`/courses/${course.slug}/chapters/${chapter.id}`} className="w-full sm:w-auto">
              {isChapterFullyComplete ? (
                ...
                <PrimaryButton
                  className="w-full sm:w-auto chapter-card-button text-sm sm:text-base px-3 sm:px-4 py-2"
                  data-tour="start-chapter-btn"
                >
                  Continue
                </PrimaryButton>
              ) : (
                <InteractiveHoverButton
                  className="relative w-full flex items-center justify-center chapter-card-button rounded-lg sm:rounded-xl font-semibold px-3 py-2 sm:px-4 sm:py-3 bg-gray-200 text-black hover:bg-black hover:text-white transition-all duration-300 ease-in-out shadow-sm text-sm sm:text-base"
                  data-tour="start-chapter-btn"
                >
                  ㅤㅤㅤStart Chapter
                </InteractiveHoverButton>
      ...
      <CardContent className="chapter-card-content space-y-1 sm:space-y-2 pb-0 sm:pb-2 sm:hidden" data-tour="lesson-preview">
        {chapter.lessons.slice(0, 3).map((lesson) => {
          ...
          return (
            <div key={lesson.id} className="lesson-item flex items-center gap-2 p-1 rounded-lg hover:bg-surface-elevated transition-colors">
              <div className={`w-5 h-5 rounded-full flex items-center justify-center text-xs ${isCompleted ? 'bg-success text-success-foreground' : 'bg-surface text-muted-foreground'}`}>
                {isCompleted ? 'Check' : lesson.tasks.reduce((sum, task) => sum + task.xp, 0)}
              </div>
              <span className={`text-xs flex-1 ${isCompleted ? 'text-success-foreground' : 'text-foreground'}`}>
                {lesson.name}
              </span>
            </div>
          );
        ...
        {chapter.lessons.length > 3 && (
          <div className="text-center pt-1 pb-2">
            <Link to={`/courses/${course.slug}/chapters/${chapter.id}`} className="text-xs text-primary hover:text-primary/80">
...
/* PageWrapper & ErrorState unchanged */
  ...
  return (
    <Card className="max-w-md mx-auto text-center p-2 sm:p-8">
      <CardContent>
        ...
        <h3 className="text-sm sm:text-lg font-semibold text-foreground mb-1 sm:mb-2">Course Not Found</h3>
