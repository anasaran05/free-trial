...
export default function LearningPage() {
  const { courseSlug, chapterId, lessonId, topicId } = useParams<{
    courseSlug: string;
    chapterId: string;
    lessonId: string;
    topicId?: string;
  ...
  const { run, steps, handleFinish } = useOnboarding("learning_page", email);
  ...
  useEffect(() => {
    loadLearningData();
  }, [courseSlug, chapterId, lessonId, topicId]);

  const loadLearningData = async () => {
    if (!courseSlug || !chapterId || !lessonId) return;
...
if (!foundCourse) {
  ...
  const handleTopicSelect = async (topic: Topic) => {
    ...
    navigate(`/courses/${courseSlug}/chapters/${chapterId}/lessons/${lessonId}/learning/${topic.id}`);
  };
...
const handleStartSimulation = async () => {
  try {
    ...
    // Get the actual course object using slug
    ...
    // Now use course.id (the real internal ID) for findLesson
    const lesson = findLesson(courses, course.id, chapterId!, lessonId!);

    if (!lesson || lesson.tasks.length === 0) {
      toast({ title: "No Tasks Available", description: "No tasks found for this lesson." });
      return;
    }
    ...
    if (type === "simulation") {
      navigate(`/courses/${courseSlug}/chapters/${chapterId}/tasks/${firstTask.id}/simulation`);
    } else if (type === "consulting") {
      navigate(`/courses/${courseSlug}/chapters/${chapterId}/tasks/${firstTask.id}/consulting`);
    } else {
      navigate(`/courses/${courseSlug}/chapters/${chapterId}/tasks/${firstTask.id}`);
    }
  } catch (error) {
  ...
  return (
    <>
      {/* ONBOARDING TOUR - This shows the dialog popup */}
      {run && steps.length > 0 && (
        <Joyride
          run={run}
          steps={steps}
      ...
      {/* Your exact original JSX starts here */}
      {loading ? (
            ...
            <Card className="max-w-md mx-auto text-center">
              <CardContent className="p-8">
                ...
                <div className="space-y-2 mb-4">
                  ...
                  <p className="text-xs text-muted-foreground">Course: {courseSlug}</p>
                  <p className="text-xs text-muted-foreground">Chapter: {chapterId}</p>
                  <p className="text-xs text-muted-foreground">Lesson: {lessonId}</p>
                </div>
                <Link to={`/courses/${courseSlug}/chapters/${chapterId}/lessons/${lessonId}`}>
          ...
          <div className="container mx-auto px-3 sm:px-4 py-6 sm:py-12">
            <nav className="mb-4 sm:mb-8" data-tour="breadcrumb">
              <div className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm text-muted-foreground overflow-x-auto">
                ...
                <Link to={`/courses/${courseSlug}`} className="hover:text-foreground transition-colors whitespace-nowrap">{courseName}</Link>
                <ChevronRight className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
                <Link to={`/courses/${courseSlug}/chapters/${chapterId}`} className="hover:text-foreground transition-colors whitespace-nowrap">{chapterName}</Link>
                <ChevronRight className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
                <Link to={`/courses/${courseSlug}/chapters/${chapterId}/lessons/${lessonId}`} className="hover:text-foreground transition-colors whitespace-nowrap">{lessonName}</Link>
                <ChevronRight className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2 sm:space-y-3">
                        {quizQuestions[currentQuizQuestion]?.options.map((option, index) => (
                          ...
                          >
                            <div className="text-left break-words whitespace-normal leading-relaxed">{option}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-2 sm:gap-0 justify-between">
                      <Button variant="outline" onClick={() => setShowQuiz(false)} className="w-full sm:w-auto text-sm sm:text-base px-4 sm:px-6 py-2 sm:py-2">
                        Back to Learning
                      </Button>
                      <Button
        ...
        <div className="min-h-screen bg-background">
          <div className="container mx-auto px-3 sm:px-4 py-6 sm:py-12">
            ...
            {topics.length === 0 && quizQuestions.length === 0 ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                <div className="lg:col-span-3">
                  ...
                  {quizCompleted && (
                    <Card className="mt-6 hidden lg:block" data-tour="quiz-results-desktop">
                      <CardHeader>
                      ...
                      <CardContent>
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            ...
                            <Badge variant={isPassed ? "default" : "destructive"}>{quizScore}%</Badge>
                          </div>
                          ...
                          <div className="space-y-4">
                            {isPassed ? (
                              <>
                                ...
                                <Button onClick={handleStartSimulation} className="w-full">Start Workplace Simulation</Button>
                              </>
                            ) : (
                              <>
                                <p className="text-destructive">You need 80% to pass. Try again!</p>
                                <Button onClick={handleQuizStart} variant="outline" className="w-full">Retake Quiz</Button>
                              </>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="space-y-2">
                          {topics.map((topic, idx) => (
                            <Button
                              ...
                              data-tour={idx === 0 ? "first-topic-btn" : undefined}
                            >
                  ...
                  <Card data-tour="progress-card">
                    ...
                    <CardContent>
                      <div className="space-y-4">
                        ...
                        {topics.length > 0 ? (
                          isQuizUnlocked ? (
                            <Button onClick={handleQuizStart} className="w-full" disabled={showQuiz} data-tour="take-quiz-btn">
                              {quizCompleted ? "Retake Quiz" : "Take Quiz"}
                            </Button>
                          ) : (
                            <Button disabled className="w-full">Complete All Topics to Unlock Quiz</Button>
                          )
                        ) : (
                          <Button disabled className="w-full">No Quiz Available</Button>
                        )}

                        {isPassed && (
                          <div className="text-center">
                            <Badge className="mb-2" variant="default">Quiz Passed!</Badge>
                            <p className="text-sm text-muted-foreground">You can now access simulation tasks</p>
                          </div>
                        )}
                      </div>
                    </CardContent>
            ...
            {quizCompleted && (
              <Card className="mt-6 lg:hidden" data-tour="quiz-results-mobile">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <Award className="h-5 w-5" />
                    Quiz Results
